...
public class GroupMembershipService {
  ...
  public void addUser(String userId, String groupId) {
    groups.group(groupId);                // existiert?
  ...
  public int addUsersBulk(String groupId, BulkUserIdsRequest body) {
    groups.group(groupId);
    int n = 0;
    for (String id : body.userIds()) { realm.users().get(id).joinGroup(groupId); n++; }
    return n;
  }
  ...
  public int removeUsersBulk(String groupId, BulkUserIdsRequest body) {
    groups.group(groupId);
    int n = 0;
    for (String id : body.userIds()) { realm.users().get(id).leaveGroup(groupId); n++; }
    return n;
  }
}
